    		...
    		.rfind(|(_, c)| c.is_some_and(|c| c == b'$'))
		{
    		...
    		match (|| {
    			...
    			match next_char {
    				...
    				b'!' => Some((2, Cow::Borrowed(std::slice::from_ref(&BYTES[exec.context() as usize])))),
    				...
    				c if c.is_ascii_digit() || *c == b'-' => {
    					...
    					let mut end = i + 2;
    					while let Some(byte) = source.get(end).and_then(Option::as_ref) && byte.is_ascii_digit() {
    						end += 1;
    					}
    					let source_slice = source.get(start .. end)?.iter().copied().filter_map(|v| v).collect::<Vec<_>>();
